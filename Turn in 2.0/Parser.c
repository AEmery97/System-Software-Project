...
{	
	...
	switch(code)
	{
		...
		case 5:
			...
			break;
		case 6:
			printf("Incorrect symbol after procedure declaration. (code:%d)\n", code);
			break;
		case 7:
			printf("Statement expected.(code:%d)\n", code);
			break;
		case 8:
...
void parser(char* toks, FILE* output, int assemblyFlag, int* errorFlag)
{
	...
	sp = 1;
	level = cLevel = 1;
	tokensLength = strlen(toks);
	//tokens = malloc(sizeof(char) * (tokensLength + 1));
	...
	fprintf(output, "PARSER: Assembly:\n");
...
int program()
{
	...
	emit(sio3, 0, 0, 3);// im pretty sure this should be return
...
int factor()
{
	...
	if(token == 2) //identsym
	{
		...
		if(symbolTable[index].kind == 2)
			emit(lod, reg, cLevel - symbolTable[index].level, symbolTable[index].addr - 1);
